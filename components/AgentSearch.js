...
const AgentSearch = () => {
    const services = ["Search for an agent by name or location", "Filter the results base on your interests", "Select and contact your best match agent"];
    return (
        <div className='flex flex-row bg-white p-12'>
            <div className='flex flex-col  '>
                ...
                <p className='text-sm text-textGray w-[40.7%]'>You should choose an agent you are comfortable working with and who you feel has your best interests at heart. you should be able to get your best agent by considering the following simple steps:</p>
                <div className='flex flex-col mt-12'>
                    {services.map((service, index) => (
                        <div key={index} className='flex flex-row    '> <div className='w-[16px] h-[14px] bg-optGray  rounded-b-[4px] rounded-l-[4px] mr-4 mt-2  '><div className='w-[16px] h-[14px] bg-optGray rounded-b-[4px] rounded-l-[4px] opacity-[80%]   '></div></div><span className='text-textGray text-[16px] pb-4'>{service}</span></div>
                    ))}
                ...
                <div className='flex flex-row justify-between items-center w-fit h-[51px] bg-mainblue p-[12px]  rounded-[5px] mt-8'>
                    <button className='text-white flex flex-row  items-center justify-between '>
                        <span><Image src={searchIcon} className='basis-1/2' /></span>
